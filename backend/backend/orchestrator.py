...
from backend.analyzers.gpt_eval import evaluate_video_file
...
def run_gpt(
    ...
    api.upload_json(RECORDINGS_BUCKET, paths["gpt_eval"], payload)
    apply_gpt_result(api, request.recording_id, result)
    mark_job(
        api,
        request.recording_id,
        "gpt_eval",
        "succeeded",
        artifact_path=paths["gpt_eval"],
        summary=payload,
        error=None,
        finished_at=utc_now(),
    )
