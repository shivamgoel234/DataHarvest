...
def _gpt_job(api: SupabaseApi, recording_id: str) -> dict[str, Any] | None:
    res = api.client.get(
    ...
    while time.monotonic() < deadline:
        ...
        if not row.get("is_scoring"):
            if not _valid_score_row(row):
                gpt_job = _gpt_job(api, recording_id)
                raise RuntimeError(
                    "Scoring finished without complete GPT fields for "
                    f"{recording_id}: row={json.dumps(row, sort_keys=True, default=str)} "
                    f"gpt_job={json.dumps(gpt_job, sort_keys=True, default=str)}"
                )
            return row
