...
APP_NAME = "dataharvest-backend-analysis"
MODEL_VOLUME_NAME = "dataharvest-modal-inference-models"
MODEL_ROOT = "/models"
SECRET_NAME = "dataharvest-backend-secrets"
...
def resource_intensive_analysis_enabled() -> bool:
    return os.environ.get("DATAHARVEST_ENABLE_RESOURCE_INTENSIVE_AI_TASKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def process_recording(payload: dict) -> dict[str, object]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures[executor.submit(run_gpt, api, request, task, video_bytes)] = "gpt_eval"
...
def submit_analysis(payload: dict[str, Any], request: Request):
    ...
    received = request.headers.get("X-DataHarvest-Modal-Secret")
